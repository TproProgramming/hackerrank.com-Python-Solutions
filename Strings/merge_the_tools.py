...
def merge_the_tools(string, k):
    ...
    for chunk in chunks:
        result = remove_non_distinct_letters(chunk)
        print(result)
               
def remove_non_distinct_letters(word):
    seen = set()  # Set to keep track of letters we've already added
    result = []
    
    for letter in word:
        if letter not in seen:
            result.append(letter)  # Add the first occurrence
            seen.add(letter)  # Mark the letter as seen
